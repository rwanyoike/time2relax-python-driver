...
def test_ddoc_view_with_params():
    ...
    assert result == ('POST', '_design/test/_view/myview', {
        'json': params,
    })
...
def test_get_with_params():
    ...
    assert result == ('GET', '_design/testid', {
        'params': {
...
def test_insert_att_with_headers():
    result = time2relax.insert_att(
        'doc', '1-5bfa2c9', 'att.txt', 'Zm9v', 'text/plain', headers={'X-Assert': 'true'})
    assert result == ('PUT', 'doc/att.txt', {
        'data': 'Zm9v',
        'headers': {
            'Content-Type': 'text/plain',
...
def test_remove_att():
    ...
    assert result == ('DELETE', 'doc/att.txt', {
        'params': {
            'rev': '1-5bfa2c9',
        },
    })
...
def test_replicate_to():
    ...
    assert result == ('POST', 'http://xxxx.db:5984/_replicate', {
        'json': {
            'source': source,
            'target': target,
        },
...
def test_request_with_params(mocker):
    ...
    result = time2relax.request(session, TEST_URL, 'HEAD', '', params={
        'foo': 'bar',
        'bool': True,
        'int': 21,
    })
    assert result == mock_request.return_value
    mock_request.assert_called_with(session, 'HEAD', TEST_URL, params={
        'foo': 'bar',
        'bool': 'true',
        'int': 21,
    })
